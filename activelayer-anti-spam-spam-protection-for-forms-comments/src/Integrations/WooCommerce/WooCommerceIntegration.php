...
namespace ActiveLayer\Integrations\WooCommerce;

use ActiveLayer\Helpers\SettingsHelper;
use ActiveLayer\Integrations\BaseFormIntegration;
use ActiveLayer\Integrations\WooCommerce\Registration\RegistrationIntegration;
use ActiveLayer\Integrations\WooCommerce\Reviews\ReviewsIntegration;
use LogicException;
...
/**
 * WooCommerce umbrella integration.
 *
 * Single IntegrationRegistry entry (slug 'woocommerce') that composes
 * the Reviews and Registration sub-integrations. Each sub-integration
 * remains a full BaseFormIntegration with its own slug used for storage
 * (`wc_reviews`, `wc_registration`); only this umbrella is registered.
 *
 ...
 * sub options (`activelayer_wc_reviews_settings`,
 * `activelayer_wc_registration_settings`), so there is exactly one source
 ...
 *
 * Slug `woocommerce` is registry-only. The option
 * `activelayer_woocommerce_settings` is NEVER read or written — settings
 * live on each sub-integration. The parent's `load_settings()` will populate
 * `$this->settings` from defaults on construction; that state is ignored
 * because `get_settings()` is overridden to return an aggregate.
 *
 * @since 1.2.0
 *
 * @package ActiveLayer\Integrations\WooCommerce
 */
final class WooCommerceIntegration extends BaseFormIntegration {
	...
	public function __construct() {
		...
		parent::__construct( 'WooCommerce', 'woocommerce' );
